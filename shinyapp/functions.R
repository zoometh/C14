findLocations <- function(shape, location_coordinates, location_id_colname){
  ...
  if (class(location_coordinates) != "standardGeneric"){
    ...
    print("SHAPE REMOVED")
    print(class(location_coordinates))
    print(head(location_coordinates))
    print(location_id_colname)
    x = (location_coordinates[which(is.na(selected_locs)), location_id_colname])
    selected_loc_id = as.character(x[[location_id_colname]])
    return(selected_loc_id)
  } 
...
f.gcalib <- function(some_14C,clicked_site,C14.grouped){
  ...
  # f.gcalib <- function(ages3,some_14C,clicked_site,C14.grouped){
  # TODO: make variable form parameters
  # TODO: complete with radiocarbon/spdxxx.R
  # in order to cumulated dates for i) same period, ii) same us
  ## graphical param
  ...
  if (C14.grouped == "C14all"){
    ...
    # site.per <- paste0(some_14C.a.per[1,"SiteName"],'\n',some_14C.a.per[1,"Period"])
    ...
    for(a.per in unique(some_14C$Period)){
      ...
      if(nb.c14 == 1){
        ...
        ages3 <- BchronCalibrate(ages=some_14C.a.per[,"C14Age"], 
                                 ...
                                 calCurves=c(rep('intcal13',nrow(some_14C.a.per)))
                                 # positions=c(1:nrow(some_14C.a.per)), 
                                 # calCurves=c(rep('intcal13',nrow(some_14C.a.per)))
        )
  ...
  if (C14.grouped == "C14groupsl"){
    for (a.site in unique(some_14C$SiteName)){
      ...
      # save(some_14C.a.site,file="aDF.R")
      ...
      for(a.us in unique(some_14C.a.site$PhaseCode)){
        ...
        if(nb.c14 == 1){
          ...
          ages3 <- BchronCalibrate(ages=some_14C.a.site.us[,"C14Age"], 
                                   ...
                                   calCurves=c(rep('intcal13',nrow(some_14C.a.site.us)))
                                   # positions=c(1:nrow(some_14C.a.per)), 
                                   # calCurves=c(rep('intcal13',nrow(some_14C.a.per)))
          )
          dat1.ageGrid <- ages3$Date1$ageGrid # load the 14C age
          ...
          # clicked_site <- clicked.site
          # test if clicked site on SiteName & date & code
          # is.clicked.site <- some_14C.a.per.site[,"SiteName"] == clicked_site[1]
          # is.clicked.age <- some_14C.a.per.site[,"C14Age"] == as.numeric(clicked_site[2])
          # is.clicked.code <- some_14C.a.per.site[,"LabCode"] == clicked_site[3]
      ...
      for (a.site in unique(some_14C.a.per$SiteName)){
        ...
        if(nb.c14 == 1){
          ...
          ages3 <- BchronCalibrate(ages=some_14C.a.per.site[,"C14Age"], 
                                   ...
                                   calCurves=c(rep('intcal13',nrow(some_14C.a.per.site)))
                                   # positions=c(1:nrow(some_14C.a.per)), 
                                   # calCurves=c(rep('intcal13',nrow(some_14C.a.per)))
          )
          dat1.ageGrid <- ages3$Date1$ageGrid # load the 14C age
  ...
  if (C14.grouped == "C14ungroup"){
    ages3 <- BchronCalibrate(ages=some_14C[,"C14Age"], 
                             ...
                             calCurves=c(rep('intcal13',nrow(some_14C)))
    )
  ...
  gcalib <- ggplot(dat.cumul)+
    ...
    facet_grid(means ~ ., scales="free_y", 
               switch="both", labeller = labeller(means = supp.labs)) +
    ...
    # TODO: symbology ageom_line size & alpha
    ...
    scale_x_continuous(breaks = seq(mm,MM,gcalib.bin), #
                       # breaks = seq(mm500,MM500,500),
                       # minor_breaks = seq(mm,MM,gcalib.bin),
                       # labels = seq(mm,MM,gcalib.bin),
                       sec.axis = dup_axis())+
    scale_y_continuous(sec.axis = dup_axis())+
    # scale_y_continuous(sec.axis = dup_axis(), limits = c(0,Md))+
