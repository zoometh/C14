---
title: "**EUROEVOL_R app**, mapping radiocarbon dating online"
author: "Thomas Huet"
# date: "11/12/2020"
header-includes:
  - \usepackage{float}
...
The [**EUROEVOL_R app**](https://neolithic.shinyapps.io/Euroevol_R/) is an interactive web application based on the **[EUROEVOL database](#db)** radiocarbon chronicles. It allows the selection of radiocarbon dating by location, chronology and material life duration by subsetting a radiocarbon dataset according to: 
...
## The **EUROEVOL\_R app** {#app}

The [**EUROEVOL_R app**](https://neolithic.shinyapps.io/Euroevol_R/) has been developped with [RShiny](https://shiny.rstudio.com/).

<p style="text-align: center;"> ![](https://raw.githubusercontent.com/zoometh/C14/main/docs/imgs/app_euroevol_thumbnail.png){width=50%}
</p>

**EUROEVOL_R app** has a different dataset from the **[NeoNet app](https://neolithic.shinyapps.io/NeoNet/)**  but the same functionalities. So, to learn how to use these two apps, visit the [**NeoNet app** webpage](https://zoometh.github.io/C14/neonet/#app).  
...
Below, we present how these data can be readed with R and adapted to serve the [**EUROEVOL_R app**](https://neolithic.shinyapps.io/Euroevol_R/)
...
* **ages**: a age (in BP)
* **ageSds**: a standard deviation
* **calCurves**: the calibration curve used
* **ageGrid**: a vector with all the years present in the calibration range (in BP)
...
kable(df.C14.sample,"html") %>%
  kable_styling(full_width = FALSE, position = "center", font_size=10)
```

Here, our purpose was calculate *tpq* and *taq* for subsetting the dataset in the [**EUROEVOL_R app**](https://neolithic.shinyapps.io/Euroevol_R/) with the **slider tpq/taq** of the [**map** panel](https://zoometh.github.io/C14/neonet/#panel.map). But for each date selected in the app, such a calibration will be performed on-the-fly and the whole density curve will be displayed on the [**calib** panel](https://zoometh.github.io/C14/neonet/#panel.calib)  

Once calculated, the dataset is saved 
